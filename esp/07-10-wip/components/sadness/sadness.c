...
esp_err_t sadness_init(void)
{
    ...
    if(personality[3]=='J'){
        ...
        ESP_LOGI(TAG, "Qualcosa è andato storto con sadness nel calcolo di alpha");
    }    
    ...
    if(personality[1]=='S'){
        ...
        ESP_LOGI(TAG, "Qualcosa è andato storto con sadness nel calcolo di inclination");
    }    
    //printf("\nSadness parameters are initialized\n");
    return ESP_OK;
}
...
void sadness_modify(float action, const char *action_name){
    ...
    if (strcmp(previous_action, action_name)==0){
        ...
        ESP_LOGI(TAG, "Qualcosa è andato storto con sadness nel calcolo di gamma");
    }
    if(strcmp("hug",action_name)==0||strcmp("caress",action_name)==0||strcmp("bug",action_name)==0){
        beta=-beta;
    }
    ...
    if( mood=='n'||mood=='N'){
       emotion[4]+=( emotion[4]*alpha + (fabsf((1-alpha)))*(action*(mood_int)+beta*inclination));   
    }
    else if (mood=='p'||mood=='P'){
/*      printf("emotion[4] at t-1: %.3f\n",emotion[4]);
        printf("alpha: %.3f\n", alpha);
        printf("mood: %.3f\n",mood_int);
        printf("emotion[4]*alpha : %.3f\n", emotion[4]*alpha);
        printf("fabs(1-%.3f): %.3f\n",alpha, fabs(1-alpha));
...
*/      emotion[4]+=( emotion[4]*alpha+ fabsf((1-alpha))*(action*(1-mood_int)+beta*inclination) );  

    }
    else {
        ESP_LOGI(TAG, "Qualcosa è andato storto con sadness nel calcolo del valore di emotion[3]");

    }
//    printf("emotion[4] at t: %.3f\n",emotion[4]);
    if (emotion[4]<=0){
        emotion[4]=0;
    }else if (emotion[4]>=1){
        emotion[4]=1;
...
void pensiveness(void){
    printf("\033[35mPensiveness triggered\033[0m\n");
    if (emotion_running) {
...
void sadness(void){
    printf("\033[35mSadness triggered\033[0m\n");
    if (emotion_running) {
        printf("an emotion is already running — skipping\n");
        return;
    }
...
void grief(void){
    printf("\033[35mGrief triggered\033[0m\n");
    if (emotion_running) {
        printf("an emotion is already running — skipping\n");
        return;
    }
